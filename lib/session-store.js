...
function createStore({ db, stmts, persistSession, persistTool, persistCompaction,
  ...
  const hiddenSessions = new Set(); // UI-only hide — no DB/file changes, cleared on restart
  const tombstoned     = new Set(); // permanently deleted IDs — persisted in deleted_sessions table
  try { for (const row of stmts.listTombstones.all()) tombstoned.add(row.id); } catch {}
  ...
  let   discoverPending = false; // debounce flag for discoverChildSessions
  ...
  function loadSessionTree(sessionId) {
    ...
    if (tombstoned.has(sessionId)) return null;
  ...
  function backfillCosts() {
    for (const node of sessions.values()) {
      ...
      const needsSubscriptionCheck = !node.isSubscription && node.tokens.input > 0;
      ...
      if (!needsCost && !needsSubscriptionCheck && !needsMeta && !needsLabel && !needsCompacts) continue;

      const transcript = findTranscript(node.sessionId, node.cwd);
      if (!transcript) continue;

      let changed = false;
      if (needsCost || needsSubscriptionCheck) {
        ...
        if (stats.inputTokens > 0 || stats.outputTokens > 0) {
          ...
          if (needsCost || (needsSubscriptionCheck && stats.isSubscription)) {
  ...
  function discoverChildSessions() {
    ...
    // Pass 1: UUID-based re-linking — scan each session's transcript for Agent tool results
    // that reference known root sessions, and re-link them as children.
    for (const node of [...sessions.values()]) {
      if (node.isSidechain || node.isWorktree) continue;
      const transcript = findTranscript(node.sessionId, node.cwd);
      if (!transcript) continue;
      try {
        const childIds = findChildSessionIds(transcript, node.sessionId);
        for (const childId of childIds) {
          if (!isValidSessionId(childId)) continue;
          const childNode = sessions.get(childId);
          if (childNode && !childNode.parentSessionId) {
            // Re-link this root session as a child
            childNode.parentSessionId = node.sessionId;
            const idx = sessionList.indexOf(childId);
            if (idx >= 0) sessionList.splice(idx, 1);
            if (!node.children.includes(childId)) node.children.push(childId);
            // Remove synthetic placeholders for this parent
            node.children = node.children.filter(cid => {
              if (cid.startsWith('child-') || cid.startsWith('sc-')) {
                sessions.delete(cid);
                return false;
              }
              return true;
            ...
            console.log(`  Re-linked ${childId.slice(0,8)} as child of ${node.sessionId.slice(0,8)}`);
          }
        }
      } catch {}
    }
    ...
    try {
      for (const dirEntry of fs.readdirSync(PROJECTS_DIR)) {
        ...
        for (const file of files) {
          ...
          if (tombstoned.has(childId)) continue; // user permanently deleted this session
    ...
    for (const node of [...sessions.values()]) {
      ...
      for (const call of agentCalls) {
        ...
        const isError    = /^Error:/.test(resultText) || /^(?:Permission denied|Access denied)/i.test(resultText);
  ...
  function evictOldSessions() {
    ...
    function evictTree(id) {
      const n = sessions.get(id);
      if (!n) return;
      for (const cid of n.children) evictTree(cid);
      sessions.delete(id);
    }
    for (const node of toEvict) {
      evictTree(node.sessionId);
  ...
  function refreshLiveCosts() {
    ...
    for (const node of sessions.values()) {
      ...
      try {
        const stats = parseFullSessionStats(transcript, node.sessionId);
        if (stats.costUsd !== node.costUsd || stats.inputTokens !== node.tokens.input || stats.isSubscription !== node.isSubscription) {
          node.tokens.input     = stats.inputTokens;
          node.tokens.output    = stats.outputTokens;
          node.tokens.cacheRead = stats.cacheReadTokens;
          node.costUsd          = stats.costUsd;
  ...
  // serialization
  function serializeSession(sessionId, _visited = new Set(), lean = false) {
    ...
    return {
      ...
      parentSessionId: n.parentSessionId || null,
      label:          n.label,
      status:         n.status,
      ...
      toolCalls: (lean ? n.toolCalls.slice(-200) : n.toolCalls).map(tc => ({
        id: tc.id, name: tc.name, done: tc.done,
        summary: tc.summary,
        ...(lean ? {} : { input: tc.input }),
        durationMs: tc.durationMs,
        startedAt: tc.startedAt || null,
        childSessionId: tc.childSessionId || null,
      })),
      children: n.children.map(cid => serializeSession(cid, _visited, lean)).filter(Boolean),
    };
  }

  function serializeTree(rootId = null) {
    const roots = rootId ? [rootId] : [...sessionList].reverse();
    const serializedRoots = roots.map(id => serializeSession(id, new Set(), true)).filter(Boolean);
  ...
  function getOrCreate(sessionId, parentSessionId = null, label = null) {
    if (hiddenSessions.has(sessionId) || tombstoned.has(sessionId)) return null;
    if (!sessions.has(sessionId)) {
      ...
      if (!parentSessionId) {
        ...
        if (sessionList.length > 200) sessionList.splice(0, sessionList.length - 200);
      } else {
        const parent = sessions.get(parentSessionId);
        if (parent && !parent.children.includes(sessionId)) parent.children.push(sessionId);
      }
    } else if (parentSessionId) {
      // Session already exists — re-link as child if it was previously a root
      const node = sessions.get(sessionId);
      if (!node.parentSessionId) {
        node.parentSessionId = parentSessionId;
        const idx = sessionList.indexOf(sessionId);
        if (idx >= 0) sessionList.splice(idx, 1);
        const parent = sessions.get(parentSessionId) || getOrCreate(parentSessionId, null);
        if (!parent.children.includes(sessionId)) parent.children.push(sessionId);
        // Remove matching synthetic child-{} placeholders from parent, carrying label to real node
        parent.children = parent.children.filter(cid => {
          if (cid.startsWith('child-') || cid.startsWith('sc-')) {
            const placeholder = sessions.get(cid);
            if (placeholder?.label && node.label === `agent-${sessionId.slice(0, 8)}`) {
  ...
  function handleHook(ev) {
    ...
    if (!sid) return;
    if (hiddenSessions.has(sid)) return; // UI-hidden, ignore further events
    ...
    if (ev.permission_mode || ev.permissionMode) {
      ...
      const newMode = ev.permission_mode || ev.permissionMode;
      if (newMode && newMode !== node.permissionMode) {
        node.permissionMode = newMode;
        persistSession(node);
        changed = true;
      }
    }
    ...
    switch (event) {
      ...
      case 'PreToolUse': {
        ...
        const agentDesc = agent_description || tool_input?.description || tool_input?.prompt || '';
        if ((tool_name === 'Agent' || tool_name === 'Task') && agentDesc) {
          const childId = `child-${tool_use_id}`;
          getOrCreate(childId, sid, agentDesc.replace(/\s+/g, ' ').slice(0, 60));
      ...
      case 'PostCompact': {
        ...
        ) || (!tool_use_id ? [...node.compactions].reverse().find(x => x.tokensAfter === null && x.summary === '') : null);
        if (existing) {
      ...
      case 'UserPromptSubmit': {
        const node = getOrCreate(sid, parentSid || null);
        // Resume a session that was ended by a previous Stop event
      ...
      case 'SubagentStop': {
        ...
        // Re-run discovery so newly completed subagents get linked to their parent in real-time
        if (!discoverPending) {
          discoverPending = true;
          setTimeout(() => { discoverPending = false; try { discoverChildSessions(); broadcast(); } catch {} }, 1500);
        }
        break;
      }
  ...
  return {
    sessions, toolCallMap, sessionList, clients, hiddenSessions,
    ...
    renameSession, hideSession, deleteSession,
  };
  ...
  function hideSession(sessionId) {
    ...
    if (node) {
      ...
      if (node.parentSessionId) {
        const parent = sessions.get(node.parentSessionId);
        if (parent) parent.children = parent.children.filter(c => c !== sessionId);
      }
      // Remove from root list
      const idx = sessionList.indexOf(sessionId);
      if (idx !== -1) sessionList.splice(idx, 1);
      // Remove the node and all its descendants
      function removeTree(sid) {
        const n = sessions.get(sid);
        if (!n) return;
        for (const child of (n.children || [])) removeTree(child);
        sessions.delete(sid);
        hiddenSessions.add(sid);
      }
  ...
  function deleteSession(sessionId) {
    ...
    // Collect all descendant IDs from in-memory tree AND DB (covers unloaded children)
    const idsToDelete = new Set();
    function collectFromMemory(sid) {
      if (idsToDelete.has(sid)) return;
      idsToDelete.add(sid);
      const n = sessions.get(sid);
      if (n) for (const child of (n.children || [])) collectFromMemory(child);
    }
    function collectFromDb(sid) {
      if (idsToDelete.has(sid)) {
        // Even if already added via memory, still walk DB children in case some weren't loaded
      }
      idsToDelete.add(sid);
      let rows = [];
      try { rows = stmts.getChildren.all(sid); } catch {}
      for (const row of rows) collectFromDb(row.id);
    }
    collectFromMemory(sessionId);
    collectFromDb(sessionId);

    // Detach from parent's in-memory children list
    const root = sessions.get(sessionId);
    if (root?.parentSessionId) {
      const parent = sessions.get(root.parentSessionId);
      if (parent) parent.children = parent.children.filter(c => c !== sessionId);
    }
    const rootIdx = sessionList.indexOf(sessionId);
    if (rootIdx !== -1) sessionList.splice(rootIdx, 1);

    // Drop in-memory state and tool-call map entries
    for (const sid of idsToDelete) {
      sessions.delete(sid);
      hiddenSessions.add(sid); // prevent re-discovery in this daemon run
      for (const [toolId, entry] of toolCallMap) {
        if (entry?.sessionId === sid) toolCallMap.delete(toolId);
      }
    }

    // Delete from DB inside a transaction. SQLite doesn't accept array bindings,
    // so build an `id IN (?, ?, ...)` placeholder list. Also write tombstones so
    // discoverChildSessions() won't re-import these from the JSONL files on next scan.
    const ids = Array.from(idsToDelete);
    if (ids.length) {
      const placeholders = ids.map(() => '?').join(',');
      const tables = ['tool_calls', 'compactions', 'permission_decisions',
                      'network_requests', 'file_accesses'];
      const now = Date.now();
      const txn = db.transaction(() => {
        for (const t of tables) {
          db.prepare(`DELETE FROM ${t} WHERE session_id IN (${placeholders})`).run(...ids);
        }
        db.prepare(`DELETE FROM sessions WHERE id IN (${placeholders})`).run(...ids);
        for (const id of ids) stmts.insertTombstone.run(id, now);
      });
      try { txn(); } catch (e) { console.warn('[deleteSession] DB delete failed:', e.message); return false; }
